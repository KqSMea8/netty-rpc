...
public class RpcServer extends AbstractRpcRemotingServer implements ServerMessageSender {
    ...
    public void init() {
        ...
        System.out.println("启动成功:port:"+this.getListenPort());
        super.start();
    ...
    public void channelRead(final ChannelHandlerContext ctx, Object msg) throws Exception {
        System.out.println("从channel读数据["+ Calendar.getInstance().getTime() +"]:"+msg);
        if (msg instanceof RpcMessage) {
            ...
            if (rpcMessage.getBody() instanceof RegisterTMRequest) {
                ...
                System.out.println("接收到TM端请求["+ Calendar.getInstance().getTime() +"]:"+request.toString());
            ...
            if (rpcMessage.getBody() == HeartbeatMessage.PING) {
                System.out.println("接收到心跳PING消息["+ Calendar.getInstance().getTime() +"]:"+rpcMessage.getBody().toString());
